...
int XML_SetHDR(char* path, char* argument, cyg_uint32 bufAddr, cyg_uint32* bufSize, char* mimeType, cyg_uint32 segmentCount)
{
    UINT32            cmd=0,par=0;
    char*             pch=0;
    UINT32            ret =0;
    ...
    if(strncmp(argument,CMD_STR,strlen(CMD_STR))==0)
    {
        ...
        if(pch)
        {
            if(strncmp(pch,PAR_STR,strlen(PAR_STR))==0)
            {
                ...
                debug_msg("cmd:%d,par:%d..\r\n",cmd,par);
                if(cmd==WIFIAPP_CMD_MOVIE_HDR)
                {
                    if((par>=0)&&(par<MOVIE_HDR_ID_MAX))
                    UI_SetData(FL_MOVIE_HDR, par);
                }
            }
        }                
    }         
    XML_ValueResult(WIFIAPP_CMD_MOVIE_HDR,par,bufAddr,bufSize,mimeType);
    
    return CYG_HFS_CB_GETDATA_RETURN_OK;    
}

int XML_SetMotionDet(char* path, char* argument, cyg_uint32 bufAddr, cyg_uint32* bufSize, char* mimeType, cyg_uint32 segmentCount)
{
    UINT32            cmd=0,par=0;
    char*             pch=0;
    UINT32            ret =0;
    ...
    if(strncmp(argument,CMD_STR,strlen(CMD_STR))==0)
    {
        ...
        if(pch)
        {
            if(strncmp(pch,PAR_STR,strlen(PAR_STR))==0)
            {
                sscanf(pch+strlen(PAR_STR),"%d",&par);
                debug_msg("cmd:%d,par:%d..\r\n",cmd,par);
                if(cmd==WIFIAPP_CMD_MOTION_DET)
                {
                    if((par>=0)&&(par<MOVIE_MOTIONDET_ID_MAX))
                    {
    ...
    XML_ValueResult(WIFIAPP_CMD_MOTION_DET,par,bufAddr,bufSize,mimeType);
    
    return CYG_HFS_CB_GETDATA_RETURN_OK;    
}

int XML_SetLiveViewSize(char* path, char* argument, cyg_uint32 bufAddr, cyg_uint32* bufSize, char* mimeType, cyg_uint32 segmentCount)
{
    UINT32            cmd=0,par=0;
    char*             pch=0;
    UINT32            ret =0;
    ...
    if(strncmp(argument,CMD_STR,strlen(CMD_STR))==0)
    {
        ...
        if(pch)
        {
            if(strncmp(pch,PAR_STR,strlen(PAR_STR))==0)
            {
                sscanf(pch+strlen(PAR_STR),"%d",&par);
                debug_msg("cmd:%d,par:%d..\r\n",cmd,par);
                if(cmd==WIFIAPP_CMD_MOVIE_LIVEVIEW_SIZE)
                {
                    if((par>=0)&&(par<WIFI_MOVIE_APP_PREVIEW_SIZE_ID_MAX))
                    UI_SetData(FL_WIFI_MOVIE_APP_PREVIEW_SIZE, par);
                }
            }
        }                
    }         
    XML_ValueResult(WIFIAPP_CMD_MOVIE_LIVEVIEW_SIZE,par,bufAddr,bufSize,mimeType);
