...
INT32 SetupExe_OnSensorRotate(VControl *pCtrl, UINT32 paramNum, UINT32 *paramArray)
{
    UINT32 uhSelect = 0;
    UINT32 uiCurMode,uiChangeTo = 0;
    UINT32 i;	
    ...
    if(paramNum>0)
        uhSelect= paramArray[0];
    if(UI_GetData(FL_SENSOR_ROTATE)!=uhSelect)	
    {
    	UI_SetData(FL_SENSOR_ROTATE,uhSelect);
       uiCurMode = UI_GetData(FL_NextMode);
       uiChangeTo = uiCurMode;
        UI_SetData(FL_PreMode,uiCurMode);
        UI_SetData(FL_NextMode,uiChangeTo);
...
//Mode
    ...
    {NVTEVT_EXE_SENSOR_ROTATE,             SetupExe_OnSensorRotate       },
