...
EVENT_ITEM(NVTEVT_KEY_WIFIONOFF,UIFlowWndMovie_OnWifiOnOFF)
...
static BOOL gbNeedToUpdateMcuFW=TRUE;
UINT8 *McuFWBuf;
extern void SendPushMessage(int eventType);
static BOOL UserUpgradeMCUFWDoing();
static BOOL FlowMovie_OnTimerUpdateMcu();

#define UPDATE_FILE  "A:\\MCU.BIN"

BOOL UIFlowMovie_CheckUpdateMcu(void)
{
  BOOL ret = FALSE;
  FST_FILE filehdl = NULL;
     ...
     if (filehdl!=NULL)
     {
	    debug_msg("detect mcu bin file ..\r\n");     
           FileSys_CloseFile(filehdl);
	    ret = TRUE;	     
     }
     else
     {
	    debug_msg("detect no mcu bin file ..\r\n");     
     }
  return ret;
}

BOOL UserUpgradeMCUFWRequire()
{
	UINT32 i = 0;
       UINT8 Cmd_Return[22]={0};
	UINT32 checkSum=0;
	
      debug_msg("Uart2 send data :");
	Cmd_Return[0]=0x5A;
	Cmd_Return[1]=0xA5;
	Cmd_Return[2]=0xB0;
	Cmd_Return[3]=0x01;
	...
	for(i=0;i<21;i++)
	{
		checkSum+=Cmd_Return[i];		
	}       
	checkSum=checkSum&0XFF;
	
	checkSum=(~checkSum)+1;
	Cmd_Return[21]=checkSum;
	
	for(i=0;i<22;i++)
	{
		uart2_putChar(Cmd_Return[i]);		
		debug_msg("0x%02x ",Cmd_Return[i]);		
	}
      debug_msg("\r\n");
}

BOOL UserUpgradeMCUFWTransmitOneFrame(UINT8 *SourceDat)
{
	UINT32 i = 0;
       UINT8 Cmd_Return[22]={0};
...
BOOL UserUpgradeMCUFWDoing()
{
	...
	debug_msg("file size = %X\r\n",uiFileSize);
	McuFWBuf = (UINT8 *)OS_GetMempoolAddr(POOL_ID_APP);
	if (FileSys_ReadFile(pFile,(UINT8 *)McuFWBuf,&uiFileSize,FST_FLAG_NONE,NULL)!=FST_STA_OK)
	{
		debug_msg("file read failed\r\n");
		return FALSE;
	}
 
	FileSys_CloseFile(pFile);	
	loop=(uiFileSize%16==0)?(uiFileSize/16):((uiFileSize/16)+1);
	for(i=0;i<loop;i++)
	{
		UserUpgradeMCUFWTransmitOneFrame(McuFWBuf);
		Delay_DelayMs(100);
	}
#endif
    return TRUE;
...
BOOL FlowMovie_OnTimerUpdateMcu()
{
      MCUINFO McuInfo;
...
void UIFlowwndDeleteFWFile(void)
{
     char        *fileName;
     UINT32 ret;
#if (_MODEL_DSC_ == _MODEL_SL508_)	 
     fileName = "A:\\FWSL508A.bin";    
#else
     fileName = "A:\\FW96655A.bin";    
#endif
     ret=FileSys_DeleteFile(fileName);
     if(FST_STA_OK ==ret)
     debug_msg("---UIFlowwndDeleteFWFile..OK\r\n");
     else
     debug_msg("---UIFlowwndDeleteFWFile..NG\r\n");        
}
...
INT32 UIFlowWndMovie_OnKeyMenu(VControl *pCtrl, UINT32 paramNum, UINT32 *paramArray)
{
    ...
    debug_msg("UIFlowWndMovie_OnKeyMenu:%d..%d..\r\n",gMovData.State,MOV_ST_VIEW);
    switch (uiKeyAct)
    {
      case NVTEVT_KEY_PRESS:
        switch(gMovData.State)
...
{
    UINT32  uiSoundMask;
debug_msg("UIFlowWndMovie_OnChildClose..%d..\r\n",gMovData.State);
...
INT32 UIFlowWndMovie_OnWifiOnOFF(VControl *pCtrl, UINT32 paramNum, UINT32 *paramArray)
{
    UINT32  uiKeyAct;
    UINT32 i;
    uiKeyAct = paramArray[0];
