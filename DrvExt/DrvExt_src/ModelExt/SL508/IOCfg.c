...
GPIO_INIT_OBJ uiGPIOMapInitTab[] = {
    ...
    {  GPIO_RED_LED,          GPIO_DIR_OUTPUT,    GPIO_SET_OUTPUT_LOW,  PAD_RED_LED         },
...
void GPIOMap_SetWatchDogStatus(BOOL Flag)
{
	...
	//debug_msg("GPIOMap_SetWatchDogStatus:%d..\r\n",gpio_getPin(PWMID_WATCHDOG_PCTL));	
}

BOOL GPIOMap_AccDet(void)
{
    return (gpio_getPin(GPIO_DC_DET)? FALSE : TRUE);
}

BOOL  GPIOMap_GetWifiPower(void)
{
    return (gpio_getPin(GPIO_WIFI_POWER_PWM5)? TRUE : FALSE);
}

void GPIOMap_SetWifiPower(BOOL En)
{
	if(En)
	gpio_setPin(GPIO_WIFI_POWER_PWM5);
	else
